...
import file from "./source_16.txt";
import testFile from "./test_16.txt"
...
/**
 * take binary string and return list of packets
 * @param input
 */
const parsePacket = (input, totalSubPackets = -1) => {
    const packets = []
    let numberOfPackets = 0
    const startInputSize = input.length
    while(input.length > 0 && (totalSubPackets < 0 || numberOfPackets < totalSubPackets)) {
        if(/^0+$/.test(input)) {
            break
        }
...
const sumVersion = (packets) => {
    return packets
        .map(packet => packet.version + sumVersion(packet.packets))
...
const assertEqual = (a, b) => {
    if(a === b) {
        ...
        console.log(" 😭 Opsss: ", a, b)
    }
}



export const exercise_31 = async () => {
    fetch(file)
        .then( r => r.text() )
        .then( t => {
            let input = t.trim();
